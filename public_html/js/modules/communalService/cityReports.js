/*
*	CityHub - Communal service - City reports
*/
...
$(document).ready(function() {
		...
		if (reportID != '') {
			...
			var loadCityCompanies = $.ajax({
				...
				if (data.status == '1') {
					...
					$('#reportDelegateModal').modal('hide'); 
				} else {
					var n = noty({
	...
	for (var i=0; i<reportsList.length; i++) {
		...
		var info = new google.maps.InfoWindow({
			content: "<h3 style='color: #222; text-align: center;'>" + reportsList[i].id + " </h3><br /><img style='max-width: 300px; height: auto;' src=" + reportsList[i].image + "/>"
		});

		infoWindow.push(info);

		var marker = new google.maps.Marker({
		  position: new google.maps.LatLng(reportsList[i].latitude,reportsList[i].longitude),
		  icon: reportMarkerImage 
		});

		markers.push(marker);
	
		var reportDetailButton = '#report' + reportsList[i].id + 'detailbutton';
		markers[i].setMap(map);
		google.maps.event.addListener(markers[i], 'click', function(i) {
			//console.log(reportDetailButton);
			return function() {
