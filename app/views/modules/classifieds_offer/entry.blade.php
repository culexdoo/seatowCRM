		...
		<div class="col-lg-6">
			...
			<div class="form-group">
				...
				<div class="col-xs-12 col-sm-12 col-md-12">
					...
					{{ Form::text('short_description', isset($entry->short_description) ? $entry->short_description : null, ['class' => 'form-control', 'placeholder' => 'Kratki opis']) }}
					<small class="text-danger">{{ $errors->first('short_description') }}</small>
				</div>
				<div class="col-xs-12 col-sm-12 col-md-12">
					<label for="color">Boja:</label> 
					{{ Form::text('color', isset($entry->color) ? $entry->color : null, ['class' => 'form-control', 'placeholder' => 'Boja']) }}
					<small class="text-danger">{{ $errors->first('color') }}</small>
				</div>
				<div class="col-xs-12 col-sm-12 col-md-12">
					<label for="material">Materijal:</label> 
					{{ Form::text('material', isset($entry->material) ? $entry->material : null, ['class' => 'form-control', 'placeholder' => 'Materijal']) }}
			...
			<div class="form-group">
				<div class="col-xs-12 col-sm-12 col-md-12"> 
					<label for="image" style="padding-top: 10px;">Trenutna slika:</label>
						...
						{{ HTML::image(URL::to('/') . '/uploads/modules/classifiedoffer/thumbs/' . $entry->image, $entry->title) }}
