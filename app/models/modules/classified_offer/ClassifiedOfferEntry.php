...
class ClassifiedOfferEntry extends Eloquent
{
	protected $table = 'classifiedoffer_entries';
	...
	*/
	public static function getSingleOfferEntry($entry_id = null, $paginate = false, $items = 10)
	{
		try
		{
			$entry = DB::table('classifiedoffer_entries')
				->join('classifieds_categories', 'classifieds_categories.category_id', '=', 'classifiedoffer_entries.category_id')
				->join('users', 'classifiedoffer_entries.user_id', '=', 'users.id') 
				->select(
			...
			if ($entry_id != null)
			{
				$entry = $entry->where('classifiedoffer_entries.id', '=', $entry_id)
					->first();
			...
			if ($category != null)
			{
				$entries = $entry->where('classifiedoffer_entries.category_id', '=', $category);
			}
 	...
 	public static function getSingleDemandEntry($entry_id = null, $paginate = false, $items = 10)
	{
		try
		{
			$entry = DB::table('classifieddemand_entries')
				...
				->join('users', 'classifieddemand_entries.user_id', '=', 'users.id') 
				->select(
	...
	public static function getLastClassifiedsOffersByUser($user = null, $number = 10)
	{
		if ($user == null)
		{
		...
		try
		{
			$entries = DB::table('classifiedoffer_entries')
				...
				->where('classifiedoffer_entries.user_id', '=', $user)
